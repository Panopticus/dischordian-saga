#!/usr/bin/env python3
"""SHADOW_TONGUE VO GENERATOR
Voice: 14wGKUgRFDPSwtCQurbB
Run: python3 scripts/generate_shadow_tongue_vo.py"""
import json, os, sys, time, requests, boto3

ELEVENLABS_KEY = os.environ.get("ELEVENLABS_API_KEY", "")
VOICE_ID = "14wGKUgRFDPSwtCQurbB"
BUCKET = "dgrsvoices"
REGION = "us-east-2"
S3_PREFIX = "ShadowTongue Voices"
...
EMOTIONS = {
    "corrupted": {
        ...
        "style": 0.8,
        "prefix": "*words sliding and shifting, reality being rewritten* "
    },
    "elegant": {
        "stability": 0.35,
        "similarity_boost": 0.78,
        "style": 0.55,
        "prefix": "*elegantly sinister, a poet describing atrocity* "
    },
    "amused": {
        "stability": 0.3,
        "similarity_boost": 0.75,
        "style": 0.6,
        "prefix": "*deeply amused by the player's naivety* "
    },
    "revealing": {
        "stability": 0.4,
        "similarity_boost": 0.8,
        "style": 0.45,
        "prefix": "*dropping pretense, genuinely revealing something true* "
    },
    "whispered": {
        "stability": 0.25,
        "similarity_boost": 0.72,
        "style": 0.5,
        "prefix": "*whispering from behind the words themselves* "
    }
}
...
def main():
    with open(os.path.join(os.path.dirname(__file__), "shadow_tongue-lines.json")) as f:
        lines = json.load(f)
    print(f"=== {'SHADOW_TONGUE'} VO GENERATOR ===\n  {len(lines)} lines | Voice: {VOICE_ID}\n")
    if not ELEVENLABS_KEY: print("ERROR: export ELEVENLABS_API_KEY=your_key"); sys.exit(1)
    # IDEMPOTENT_PATCH
    manifest_path = os.path.join(os.path.dirname(__file__), "..", "shared", "shadow_tongueVoManifest.json")
    try:
    ...
    manifest_path = os.path.join(os.path.dirname(__file__), "..", "shared", f"shadow_tongueVoManifest.json")
