#!/usr/bin/env python3
"""NILMORG VO GENERATOR
Voice: 7fKASPr2SR0NYifziNgu
Run: python3 scripts/generate_nilmorg_vo.py"""
...
EMOTIONS = {
    "calm": {
        ...
        "style": 0.2,
        "prefix": "*impossibly calm, watching ants, absolute control* "
    },
    "delighted": {
        "stability": 0.35,
        "similarity_boost": 0.78,
        "style": 0.55,
        "prefix": "*delighted by death, corporate joy* "
    },
    "bored": {
        "stability": 0.6,
        "similarity_boost": 0.85,
        "style": 0.1,
        "prefix": "*bored, mildly disappointed* "
    },
    "feeding": {
        "stability": 0.3,
        "similarity_boost": 0.75,
        "style": 0.6,
        "prefix": "*feeding on resonance, savoring the moment* "
    },
    "impressed": {
        "stability": 0.45,
        "similarity_boost": 0.8,
        "style": 0.35,
        "prefix": "*genuinely impressed, which is terrifying from him* "
    }
}
...
def main():
    with open(os.path.join(os.path.dirname(__file__), "nilmorg-lines.json")) as f:
        lines = json.load(f)
    print(f"=== {'NILMORG'} VO GENERATOR ===\n  {len(lines)} lines | Voice: {VOICE_ID}\n")
    if not ELEVENLABS_KEY: print("ERROR: export ELEVENLABS_API_KEY=your_key"); sys.exit(1)
    # IDEMPOTENT_PATCH
    manifest_path = os.path.join(os.path.dirname(__file__), "..", "shared", "nilmorgVoManifest.json")
    try:
    ...
    manifest_path = os.path.join(os.path.dirname(__file__), "..", "shared", f"nilmorgVoManifest.json")
