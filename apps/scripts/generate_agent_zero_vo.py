#!/usr/bin/env python3
"""AGENT_ZERO VO GENERATOR
Voice: F1waTCPWl7KpShIScYQs
Run: python3 scripts/generate_agent_zero_vo.py"""
...
EMOTIONS = {
    "clinical": {
        ...
        "style": 0.15,
        "prefix": "*flat clinical monotone, zero emotional inflection* "
    },
    "cold": {
        "stability": 0.6,
        "similarity_boost": 0.85,
        "style": 0.1,
        "prefix": "*cold, efficient, no wasted syllables* "
    },
    "rare_warmth": {
        "stability": 0.45,
        "similarity_boost": 0.8,
        "style": 0.35,
        "prefix": "*a single degree of warmth that costs her everything* "
    }
}
...
def main():
    with open(os.path.join(os.path.dirname(__file__), "agent_zero-lines.json")) as f:
        lines = json.load(f)
    print(f"=== {'AGENT_ZERO'} VO GENERATOR ===\n  {len(lines)} lines | Voice: {VOICE_ID}\n")
    if not ELEVENLABS_KEY: print("ERROR: export ELEVENLABS_API_KEY=your_key"); sys.exit(1)
    # IDEMPOTENT_PATCH
    manifest_path = os.path.join(os.path.dirname(__file__), "..", "shared", "agent_zeroVoManifest.json")
    try:
    ...
    manifest_path = os.path.join(os.path.dirname(__file__), "..", "shared", f"agent_zeroVoManifest.json")
