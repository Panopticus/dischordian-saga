  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <motion.div
        ...
        className="w-full max-w-2xl max-h-[85vh] bg-black/95 border void-border rounded-2xl overflow-hidden flex flex-col"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b void-border bg-gradient-to-r from-amber-500/10 to-transparent">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-lg tracking-[0.2em] void-text-accent">ARK COMMANDER PASS</h2>
          </div>
        ...
        <div className="px-4 py-3 border-b border-white/5">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="font-mono text-xs void-text-accent font-bold">{currentPoints}</span>
          </div>
        ...
        {!isPremium && (
          <button
            ...
            className="mx-4 my-2 flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-amber-500/20 to-amber-600/10 border void-border hover:from-amber-500/30 transition-all"
          >
            <Sparkles size={20} className="void-text-accent shrink-0" />
            <div className="flex-1 text-left">
              <p className="font-mono text-xs font-bold void-text-accent">UPGRADE TO PREMIUM</p>
              <p className="font-mono text-[10px] text-white/40">20% faster upgrades, 15% more loot, daily card pack, exclusive skins</p>
            </div>
            <ChevronRight size={16} className="void-text-accent shrink-0" />
          </button>
        )}

        {/* Premium benefits */}
        {isPremium && (
          <div className="mx-4 my-2 flex items-center gap-2 p-2 rounded-lg void-bg-sunk border void-border">
            <Crown size={14} className="void-text-accent" />
            <span className="font-mono text-[10px] void-text-accent">PREMIUM ACTIVE</span>
            ...
            return (
              <div key={tier.tier}
                className={`flex items-stretch gap-2 rounded-xl border overflow-hidden transition-all ${
                  unlocked ? "void-border bg-white/[0.02]" : "border-white/5 bg-black/20 opacity-50"
                }`}>
                {/* Tier number */}
                <div className={`w-12 flex items-center justify-center font-display text-lg ${
                  unlocked ? "void-text-accent" : "text-white/10"
                }`}>
                  {tier.tier}
                </div>
                ...
                <div className={`flex-1 p-2 ${!isPremium ? "opacity-30" : claimed ? "opacity-50" : ""}`}>
                  <p className="font-mono text-[8px] void-text-accent tracking-wider mb-1 flex items-center gap-1">
                ...
                <div className="w-16 flex items-center justify-center">
                  {canClaim ? (
                    <button onClick={() => onClaimTier(tier.tier)}
                      className="p-2 rounded-lg void-bg-sunk void-text-accent void-bg-sunk transition-colors">
                      <Gift size={16} />
                    </button>
                  ) : claimed ? (
