      ...
      switch (msg.type) {
        ...
        case "SPECTATE": {
          ...
          const safeName = rawName.length > 0
            // eslint-disable-next-line no-control-regex -- intentional: strip ASCII control chars from untrusted spectator display names
        ...
        case "SEND_SPECTATOR_CHAT": {
          ...
          const cleanedText = msg.text
            // eslint-disable-next-line no-control-regex -- intentional: strip ASCII control chars from untrusted spectator chat
            .replace(/[\u0000-\u001f]/g, "")
            .replace(/\s+/g, " ")
            .trim()
            .slice(0, 200);
