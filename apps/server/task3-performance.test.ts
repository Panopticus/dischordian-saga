...
describe("Task 3.2 — package.json scripts", () => {
  // After the apps/* split, package.json is two levels above
  // apps/server/ (was one before).
  ...
  it("exposes pnpm run optimize:images", () => {
    ...
    expect(pkgSrc).toContain("tsx apps/scripts/optimize-images.ts");
  });
