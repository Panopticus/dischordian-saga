...
describe("Phase 84 Route & Room Wiring", () => {
  it("all 10 new routes should be registered in App.tsx", async () => {
    ...
    const appContent = fs.readFileSync("apps/client/src/App.tsx", "utf-8");
    const routes = [
  ...
  it("social-hub and war-room Ark rooms should exist", async () => {
    const fs = await import("fs");
    const gameCtx = fs.readFileSync("apps/client/src/contexts/GameContext.tsx", "utf-8");
  ...
  it("all 10 routers should be wired in routers.ts", async () => {
    const fs = await import("fs");
    const routersContent = fs.readFileSync("apps/server/routers.ts", "utf-8");
    const routers = [
