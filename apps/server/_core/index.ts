...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "test") {
    ...
    // (The 29 schema-shape `bootstrap*` IIFEs that lived here are
    // removed post-0071_baseline_v1: the baseline migration creates
    // all 322 tables and 373 FKs declared in apps/db/schema.ts, so
    // the runtime DDL compensation is no longer needed. See
    // apps/db/migrations/README.md for the cutover plan. The
    // runtime-hydrate `bootstrapMysteryRegistry` above is unrelated
    // and stays.)
  }
