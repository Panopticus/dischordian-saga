...
describe("CoNexus Cover Art", () => {
  ...
  it("all cover images should be valid CDN URLs", async () => {
    ...
    for (const game of CONEXUS_GAMES) {
      ...
      expect(game.coverImage).toMatch(/\.(png|webp|avif|jpg|jpeg)$/);
    }
  });
...
describe("Content Reward Integration", () => {
  it("WatchPage should use useContentReward", async () => {
    const fs = await import("fs");
    const content = fs.readFileSync("apps/client/src/pages/WatchPage.tsx", "utf-8");
    expect(content).toContain("useContentReward");
...
describe("Lore Appearances Timeline Component", () => {
  it("should exist as a component file", async () => {
    const fs = await import("fs");
    const exists = fs.existsSync("apps/client/src/components/LoreAppearancesTimeline.tsx");
    expect(exists).toBe(true);
  });

  it("should be imported in EntityPage", async () => {
    const fs = await import("fs");
    const content = fs.readFileSync("apps/client/src/pages/EntityPage.tsx", "utf-8");
  ...
  it("should define AGE_ORDER with 6 ages", async () => {
    const fs = await import("fs");
    const content = fs.readFileSync("apps/client/src/components/LoreAppearancesTimeline.tsx", "utf-8");
...
describe("Admin Page", () => {
  it("should exist as a page file", async () => {
    const fs = await import("fs");
    const exists = fs.existsSync("apps/client/src/pages/AdminPage.tsx");
    expect(exists).toBe(true);
  });
  ...
  it("should use tRPC admin procedures", async () => {
    const fs = await import("fs");
    const content = fs.readFileSync("apps/client/src/pages/AdminPage.tsx", "utf-8");
...
describe("Player Profile Server Sync", () => {
  it("PlayerProfilePage should use tRPC for server sync", async () => {
    const fs = await import("fs");
    const content = fs.readFileSync("apps/client/src/pages/PlayerProfilePage.tsx", "utf-8");
...
describe("Main Router Integration", () => {
  it("should include admin router", async () => {
    const fs = await import("fs");
    const content = fs.readFileSync("apps/server/routers.ts", "utf-8");
    expect(content).toContain("admin");
