...
describe("companionComments — prelude/Act 1 reactive coverage", () => {
  ...
  it("has paired Elara + Human coverage for every Act 2 trigger", () => {
    for (const trigger of ACT2_TRIGGERS) {
      const matches = COMPANION_COMMENTS.filter((c) => c.trigger === trigger);
      const speakers = new Set(matches.map((c) => c.speaker));
      expect(
        matches.length,
        `no companion comments registered for Act 2 trigger "${trigger}"`,
      ).toBeGreaterThan(0);
      expect(
        speakers.has("elara") && speakers.has("human"),
        `Act 2 trigger "${trigger}" is missing one of the two voices`,
      ).toBe(true);
    }
  });
