...
describe("companionComments — prelude/Act 1 reactive coverage", () => {
  ...
  it("has paired Elara + Human coverage for every Act 3 path trigger", () => {
    for (const trigger of ACT3_TRIGGERS) {
      const matches = COMPANION_COMMENTS.filter((c) => c.trigger === trigger);
      const speakers = new Set(matches.map((c) => c.speaker));
      expect(
        matches.length,
        `no companion comments registered for Act 3 trigger "${trigger}"`,
      ).toBeGreaterThan(0);
      expect(
        speakers.has("elara") && speakers.has("human"),
        `Act 3 trigger "${trigger}" is missing one of the two voices`,
      ).toBe(true);
    }
  });
