...
describe("companionComments — prelude/Act 1 reactive coverage", () => {
  ...
  it("has paired Elara + Human coverage for every Act 6 trigger", () => {
    for (const trigger of ACT6_TRIGGERS) {
      const matches = COMPANION_COMMENTS.filter((c) => c.trigger === trigger);
      const speakers = new Set(matches.map((c) => c.speaker));
      expect(
        matches.length,
        `no companion comments for Act 6 trigger "${trigger}"`,
      ).toBeGreaterThan(0);
      expect(
        speakers.has("elara") && speakers.has("human"),
        `Act 6 trigger "${trigger}" is missing one of the two voices`,
      ).toBe(true);
    }
  });
