...
export interface CrewMissionTemplate {
   ...
   *  (`mission_tag_complete`) to validate completion. Format:
   *    "faction:<key>"   e.g. "faction:coda"
   *    "theme:<key>"     e.g. "theme:calibration"
   *    "era:<key>"       e.g. "era:pre_fall"
   *    "danger:<band>"   e.g. "danger:routine" | "danger:suicidal"
   *  Generated by proceduralMissionFactory.generateMission(). */
  tags?: string[];
}
...
export type PendingCrewSideEffect =
  ...
  | {
      /** Stamp every unread loredex carry row for the deceased member
       *  with `memorialAtCycle = deathCycle`. Drained by the server
       *  router after the tick; the entries become memorial-only. */
