...
The DMC Clone Body Companion bible files **eight DCB tickets** for follow-up engineering or design work. Each ticket is canonical-and-tracked; Stage 1 architecture and Stage 2 dialogue authoring should land each ticket before declaring the bible's mechanical surface complete.

**DCB-1 — Bond-threshold values for channel-unlock crossings.** Per §3.3 + §5.2 + §7.1 (1): the bible recommends bond-thresholds of approximately 25 / 40 / 65 / 90 for the four trust-band crossings. The actual values are bible-deferred. **Engineering surface**: Stage 1 architects should ratify the threshold values per playtesting of the canonical Awakening Protocol pacing. **Owner**: Stage 1 architects. **Blocks**: Companion canonical-pacing; the canonical *trust-driven, not time-driven* canon depends on threshold-values that produce coherent saga-time pacing.

**DCB-2 — Dismissal mechanic interaction with bond and channel-state.** Per §3.3 + §5.7 + §7.1 (2): the canonical dismissal canon asserts bond-floor-25 and irreversible channel-state. Engineering questions: (a) does dismissal pause bond decay (if any)? (b) does dismissal suspend the Companion's NpcLine selector? (c) does dismissal remove the Companion from active-companion UI slots? **Bible recommendation**: pause bond decay, suspend selector (Companion canonically does not produce expression while dismissed), remove from active-companion UI but retain in Trophy Room as canonical-set-aside. **Owner**: Stage 1 architects.

**DCB-3 — 4-tuple personality variant authoring scope.** Per §5.5 + §7.1 (3): 64 canonical variants × ~50 per-variant lines + ~250 base lines = ~3,450 lines per Companion. Open design questions: (a) can the variant axes canonically be reduced (e.g., trust-pattern axis canonically merged with alignment axis)? (b) can per-variant content canonically be procedurally generated from variant-axis-coordinates? (c) does Stage 2 canonically author all 64 variants for shipping or canonically prioritise a subset? **Bible recommendation**: canonical 4-tuple cardinality is bible-asserted as load-bearing — do not reduce; consider procedural-generation via canonical templates per axis-coordinate; Stage 2 may canonically prioritise the most-common 4-tuple coordinates and stub the others. **Owner**: Stage 1 architects + Stage 2 dialogue authors.

**DCB-4 — Canonical-protection of why-paying-is-worse-than-not-paying.** Per §3.1 + §4.2 + §7.1 (4) + §7.2 (1): the canonical-unanswered question is canonically *protected*. Engineering must canonically ensure no engine surface canonically *exposes a solution* — no UI element canonically explains, no narrative trigger canonically resolves, no lore-codex canonically answers. **Engineering surface**: a canonical-protection invariant in the engine — any attempt to canonically expose a solution must canonically be caught and rejected at engineering-review time. **Owner**: Stage 1 architects + writers' guild canonical-protection reviewer.

**DCB-5 — Post-naming dialogue bank canonical-content load.** Per §5.5 + §5.6 + §7.1 (5): the post-naming Companion canonically integrates into all four primary game surfaces with canonical-verbal content. Open authoring scope: ~3,450 lines per Companion canonically distributed. **Owner**: Stage 2 dialogue authors. **Blocks**: full Companion verbal-content delivery in shipped saga-time.

**DCB-6 — Severance Prize ceremony for ANOTHER player Stage 4 weave.** Per §3.7 + §5.6: the Companion canonically reacts to another player's Severance Prize ceremony. Stage 4 weave question: do multi-player Trench environments canonically exist? If yes, the Companion's reaction-to-another-ceremony canon canonically ships; if no, the canon is bible-asserted-but-mechanically-deferred. **Owner**: Stage 4 weave authors + multi-player environment design.

**DCB-7 — Companion death + soul-fragment re-integration narrative.** Per §3.8: the Companion's body can die in saga-time; soul-fragment canonically re-merges with the player's Potential. Open design: what is the canonical narrative event for the integration? Bible-deferred whether the integration canonically *strengthens* or *grieves* the player. **Owner**: Stage 4 weave authors. **Blocks**: full death-and-loss arc for the Companion.

**DCB-8 — In-universe player-perception of the Companion's pre-verbal channels.** Per §6.1–6.3 sample reading: the bible-asserted samples include narrator-frame articulation of the Companion's expression-meaning. Engineering question: does the saga-time UI canonically *render the narrator-frame* as part of the player's perception, or canonically *leave the player to interpret the channels alone*? Bible-recommended: canonical *toggle* in player options — narrator-frame on by default for accessibility, optional disable for players who canonically prefer to interpret without canonical-narration. **Owner**: Stage 1 UI architects.
