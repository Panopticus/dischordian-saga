...
export const COMPANION_COMMENTS: CompanionComment[] = [
  ...
  // ── WOLF-HUNT ARC — milestone reactive comments authored in C-pivot.B ──
  // The pre-pivot 12 outcome-keyed comments were retired with the
  // Hunt-the-Hero minigame (C-pivot.A.1). The replacement set is
  // milestone-keyed (kills_count, lord_lieutenant_defeated,
  // lord_fully_harvested, lycos_first_death, etc.) and lands when
  // the milestone triggers are wired by the mission reducer.
  // See apps/shared/wolfHunt/flags.ts for the flag namespace.
