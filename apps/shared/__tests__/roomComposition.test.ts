...
describe("roomComposition / facade", () => {
  it("identifies the three composite rooms", () => {
    ...
    expect(hasRoomComposite("engineering")).toBe(false);
    expect(hasRoomComposite("observation-deck")).toBe(false);
  });

  it("returns null for unsupported rooms", () => {
    expect(resolveRoomComposite("engineering", emptyGame)).toBeNull();
  ...
  it("never emits sprite ids that aren't in the per-room catalog", () => {
    for (const roomId of ["bridge", "cryo-bay", "medical-bay"] as const) {
