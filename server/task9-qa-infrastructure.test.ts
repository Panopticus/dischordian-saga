...
describe("Task 9.1 — pre-commit hook", () => {
  ...
  const installerPath = path.resolve(REPO_ROOT, "scripts/install-git-hooks.sh");
...
describe("Task 9.2 — shipped code has no lingering TODO/FIXME markers", () => {
  ...
  const serverFiles = walkTsFiles(path.resolve(REPO_ROOT, "server"));
  const clientFiles = walkTsFiles(path.resolve(REPO_ROOT, "client/src"));
  ...
  it("the specific pre-Task-9 offenders are gone", () => {
    ...
    const dbSrc = fs.readFileSync(path.resolve(REPO_ROOT, "server/db.ts"), "utf-8");
    expect(dbSrc).not.toContain("TODO: add feature queries");

    const arkSrc = fs.readFileSync(path.resolve(REPO_ROOT, "client/src/pages/ArkExplorerPage.tsx"), "utf-8");
...
describe("Task 9.3 — PageMeta component", () => {
  const src = fs.readFileSync(
    path.resolve(REPO_ROOT, "client/src/components/PageMeta.tsx"),
    "utf-8",
  );
...
describe("Task 9.3 — robots.txt", () => {
  const src = fs.readFileSync(
    path.resolve(REPO_ROOT, "client/public/robots.txt"),
    "utf-8",
  );
...
describe("Task 9.3 — sitemap.xml", () => {
  const src = fs.readFileSync(
    path.resolve(REPO_ROOT, "client/public/sitemap.xml"),
    "utf-8",
  );
...
describe("Task 9.3 — page wiring", () => {
  it("Home page uses <PageMeta>", () => {
    const src = fs.readFileSync(
      path.resolve(REPO_ROOT, "client/src/pages/Home.tsx"),
      "utf-8",
    );
  ...
  it("ArkExplorerPage uses <PageMeta>", () => {
    const src = fs.readFileSync(
      path.resolve(REPO_ROOT, "client/src/pages/ArkExplorerPage.tsx"),
      "utf-8",
    );
    expect(src).toContain('import PageMeta from "@/components/PageMeta"');
...
describe("Task 9.3 — index.html baseline meta tags unchanged", () => {
  const src = fs.readFileSync(
    path.resolve(REPO_ROOT, "client/index.html"),
    "utf-8",
  );
