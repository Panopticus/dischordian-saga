...
## Section 18 — Prelude-Wide VFX Library

This section consolidates every VFX asset referenced across Sections 3–17 into a single lookup table. **This is the authoritative list.** Any VFX named in a beat section MUST appear here. Any VFX appearing here MUST be referenced by at least one beat section. The table is organized by *functional category* rather than beat order, so engineering can see all the flashback effects together, all the light-state effects together, all the UI effects together, etc.

**Total unique VFX asset rows: ~25.** Some assets are reused across multiple beats (sepia-drain is used in Beat E and Beat J; BiometricLock state machine serves Beat E, Beat F, and is flagged for Beat J reuse; choice pillar is Beat J primary but flagged for all future Act alignment choices). The reuse is documented explicitly in the rightmost column.

### 18.1 Core flashback / memory mechanic

| VFX ID | Effect | Tech | Output | Source beat | Reuse |
|---|---|---|---|---|---|
| `vfx_sepia_drain` | Color grade transition from normal palette to sepia/yellow-brown tones over 3s (forward) and 2s (reverse). Scope-aware: `"room"` for Beat E, `"hologram"` for Beat J | CSS filter keyframes + WebGL shader fallback | `apps/client/public/art/vfx/prelude/sepia-drain.webm` (forward + reverse variants) | §10.6 Beat E | §17.6 Beat J (hologram scope); flagged for all Act-scope memory-replay content |
| `vfx_film_damage_overlay` | Subtle scratch marks, thin vertical dust line, occasional frame jumps layered over sepia imagery. Intensity scales with memory age | PixiJS sprite sheet (3 scratch variants + 1 dust loop + 1 frame-jump one-shot) | `apps/client/public/art/vfx/prelude/film-damage.webm` (loop) | §10.6 Beat E | §17.6 Beat J (slightly heavier intensity); shared with Act-scope memory-replay content |
| `vfx_diploma_ink_bloom` | Pale-gold `#fde047` soft bloom tracing across calligraphic ink marks on a framed artifact. 2-second hold. No legible text revealed | PixiJS masked bloom over bounding box | `apps/client/public/art/vfx/prelude/diploma-ink-bloom.webm` (one-shot) | §10.6 Beat E | Mess Hall only, single-use. Not reused |

### 18.2 Light-state and biometric components

This category contains all the lockbox, light, and state-machine effects. The core `BiometricLock` component (§11.6 engineering note) serves multiple beats and is the most reused component in the VFX library.

| VFX ID | Effect | Tech | Output | Source beat | Reuse |
|---|---|---|---|---|---|
| `vfx_lockbox_bio_recognize` | BiometricLock component, four-state enum (`dormant` / `pulsing` / `recognize` / `open`). Sub-1 Hz pulse in idle, 3-frame bright flash on recognize, calm steady glow on open | CSS keyframe alpha + opacity tween + PixiJS edge glow | `apps/client/public/art/vfx/prelude/lockbox-bio-recognize.webm` (idle + recognize + open variants) | §11.6 Beat F | **§10.3 Beat E** Mess Hall strongbox (dormant state only — never opens in Prelude). **§17.6 Beat J** Archives (flagged for reuse — same component powers the Log 5 memory-crystal flare). **All Act-scope content** with biometric/state locks. Engineering: this is the most reused component in the Prelude VFX library |
| `vfx_data_slate_glow` | Cyan-pulsing data-slate surface with dense glyph-grid mask, pulse slows from ~1 Hz to ~0.25 Hz after "read" | PixiJS shader + animated glyph-grid mask | `apps/client/public/art/vfx/prelude/data-slate-glow.webm` (pulsing loop + slowdown variant) | §11.6 Beat F | Briefing Room only. Not reused in Prelude |
| `vfx_memo_holo_rise` | Pale cyan holographic sheet rising from a lockbox and projecting onto a display surface, rendering three handwritten-calligraphic text bullets one at a time | CSS keyframe + PixiJS sprite rise + text-render pass | `apps/client/public/art/vfx/prelude/memo-holo-rise.webm` | §11.6 Beat F | Briefing Room only. **First of two canonical exceptions to the "no rendered text" rule** |
| `vfx_chair_rim_hot_edge` | Warm-amber `#fbbf24` directional rim-light on the askew eighth chair over 28s build + 14s hold + 3s fade | CSS keyframe alpha tween + PixiJS directional light mask | `apps/client/public/art/vfx/prelude/chair-rim-hot-edge.webm` (build + hold + fade variants) | §12.6 Beat F.5 | Briefing Room only. Beat F.5 only. New `ChairRimLight` component (distinct from BiometricLock, per §12.6 engineering note) |
| `vfx_transfer_array_amber_standby` | Steady amber-yellow `#fbbf24` standby light above the locked transfer-array hatch. Non-pulsing, non-flashing, just *on* | CSS solid color layer + soft bloom + anamorphic flare | `apps/client/public/art/vfx/prelude/transfer-array-amber-standby.webm` (loop) | §13.6 Beat G | **§17.3 Beat J** Archives (visible as distant warm point through the doorway — continuity hook). **Any Act-scope** Medical Bay re-entry |
| `vfx_witnessing_hub_hemisphere_bloom` | Cyan-white hemispheric bloom rising out of the Witnessing Hub's central well to ~1.5m height. 12 radial spokes light sequentially clockwise from 12 o'clock over 3s | Three.js volumetric + PixiJS spoke sequencer | `apps/client/public/art/vfx/prelude/witnessing-hub-hemisphere-bloom.webm` (activation one-shot + steady-state variant) | §16.6 Beat I | Steady-state variant runs continuously from Beat I onward — the Hub stays powered. **Reused in all Act-scope content where the Witnessing Hub is visible** (including Beat J's Archives, canonically adjacent to the Bridge) |
| `vfx_primary_lights_cascade` | Wave of warm sodium-gold `#fde68a` light traveling forward from the Witnessing Hub toward the panoramic viewport over 4s, lighting each duty station in sequence | CSS keyframe color grade transition with directional mask + PixiJS station-pip sequencer | `apps/client/public/art/vfx/prelude/primary-lights-cascade.webm` (forward cascade + reverse dim-back variants) | §16.6 Beat I | Bridge only. Beat I only. Single most dramatic VFX in the Prelude |
| `vfx_warm_dust_drift` | Dust motes drifting under warm sodium-gold light instead of cold cyan — same dust, different illumination | Material swap on existing dust particle system | N/A (no new asset — lighting override) | §16.6 Beat I | Bridge only. Engineering: ensure dust particles do NOT reset when the cascade fires — same particles persist through the palette shift |
| `vfx_viewport_polarization_lift` | Panoramic viewport opacity drops from ~85% to ~65% when the cascade reaches it at 28s, revealing more of the debris graveyard | CSS opacity keyframe on viewport mask layer | `apps/client/public/art/vfx/prelude/viewport-polarization-lift.webm` (one-shot) | §16.6 Beat I | Bridge only. Beat I only |

### 18.3 NPC Inbox UI system

This category contains the Beat H NPC Inbox UI assets. The `NPCInboxUI` component is flagged for **heavy reuse** — every subsequent message in the game (hundreds of messages across Acts 1–5) uses the same visual pattern the player learns in Beat H. Engineering should treat this category as load-bearing for the full game's inbox system, not just the Prelude.

| VFX ID | Effect | Tech | Output | Source beat | Reuse |
|---|---|---|---|---|---|
| `vfx_signal_intake_lit_panel` | Cyan `#22d3ee` lit panel in the signal intake matrix, pulsing at sub-0.25 Hz to suggest active long-range listening | CSS keyframe alpha tween on 6 instances | `apps/client/public/art/vfx/prelude/signal-intake-lit-panel.webm` (loop) | §14.6 Beat H | Comms Array only in Prelude. 6 instances. Reused in any Act-scope content showing the signal intake matrix |
| `vfx_inbox_envelope_unfold` | Cyan envelope glyph expanding from idle size (~60cm) to reader size (90cm × 120cm), unfolding into a three-region UI pattern (sender header + separator + body) | PixiJS shader animation + sprite transform | `apps/client/public/art/vfx/prelude/inbox-envelope-unfold.webm` (one-shot transition) | §14.6 Beat H | **CORE NPC INBOX UI ASSET.** Reused every time a new message arrives in the player's inbox across the entire game. Must be engineered as a scalable configurable component accepting sender name, timestamp, and message body as parameters. Register in `apps/client/src/components/npcInbox/` |
| `vfx_inbox_edge_sentence_bloom` | Subtle cyan glow brightening on the final sentence of a rendered message when the VO track crosses a specified timestamp | PixiJS masked bloom over last-sentence bounding box | `apps/client/public/art/vfx/prelude/inbox-edge-sentence-bloom.webm` (one-shot) | §14.6 Beat H | Configurable per-message via `highlightSentenceIndex` parameter. Any future Inbox message can trigger this effect on any sentence |
| `vfx_amber_counter_glyph` | Steady amber `#fbbf24` unread-count counter above the Inbox envelope glyph | Static sprite + soft bloom + anamorphic flare | `apps/client/public/art/vfx/prelude/amber-counter-glyph.webm` (static) | §14.6 Beat H | **CANONICALLY THE UNREAD-COUNT INDICATOR.** Reused anywhere in the game that shows an Inbox unread count. Amber color is canonical |

### 18.4 Ambient / environmental effects

These are the smaller atmospheric effects spread across multiple beats. Individually minor, collectively responsible for the Prelude's visual continuity.

| VFX ID | Effect | Tech | Output | Source beat | Reuse |
|---|---|---|---|---|---|
| `vfx_starlight_shaft_dust` | Volumetric starlight shaft from ceiling louver with dust column drifting upward through the light | Three.js volumetric light + particle column | `apps/client/public/art/vfx/prelude/starlight-shaft-dust.webm` (loop) | §8.6 Beat D | Cargo hold only |
| `vfx_mission_slate_glow` | Cyan slate glow with sub-1 Hz pulse and saturate-on-highlight transition | PixiJS shader | `apps/client/public/art/vfx/prelude/mission-slate-glow.webm` (loop + transition variant) | §8.6 Beat D | Reused for any active Trade Empire mission board posting in Acts 1+ |
| `vfx_mission_glyph_bloom` | Small cyan holographic glyph blooms in front of a highlighted mission slate | PixiJS sprite + bloom | `apps/client/public/art/vfx/prelude/mission-glyph-bloom.webm` (one-shot) | §8.6 Beat D | One-shot per Trade Empire posting. Reused in Acts 1+ |
| `vfx_galley_pilot_warm` | Warm-amber pilot light on stove with subtle 5% brightness increase over 4s | CSS keyframe alpha tween | `apps/client/public/art/vfx/prelude/galley-pilot-warm.webm` | §9.6 Beat D.5 | Galley only. **The first warm-color light in the Prelude** — singular |
| `vfx_galley_steam_residue` | Faint vertical steam ribbon rising from coffee mug, very subtle | PixiJS particle column with low alpha | `apps/client/public/art/vfx/prelude/galley-steam-residue.webm` | §9.6 Beat D.5 | Galley only. Optional polish — cut if it weakens realism |
| `vfx_med_pod_faint_pulse` | Slow sub-0.5 Hz cyan pulse on the fourth med-pod's interior, "still running on the last watts of a dying standby cell" | CSS keyframe alpha tween on PixiJS masked sprite | `apps/client/public/art/vfx/prelude/med-pod-faint-pulse.webm` (loop) | §13.6 Beat G | Medical Bay only |
| `vfx_neural_rig_idle_hum_visual` | Very subtle ambient shimmer on neural-rig cyan status pip | PixiJS shader micro-shimmer | `apps/client/public/art/vfx/prelude/neural-rig-idle-hum.webm` (loop) | §13.6 Beat G | Medical Bay only. Pair with ambient neural-rig hum audio bed |
| `vfx_memo_paper_drift` | Single postcard-sized paper drifting slowly from a shelf to the floor over ~5s, caught in minimal ventilation airflow | PixiJS sprite with physics curve (gravity + slight horizontal drift + rotation) | `apps/client/public/art/vfx/prelude/memo-paper-drift.webm` (one-shot) | §15.6 Beat H.5 | Comms Array only. Beat H.5 only |
| `vfx_elara_fade_out` | Elara's cyan-edged hologram fading from full opacity to zero over 2s | CSS opacity keyframe | `apps/client/public/art/vfx/prelude/elara-fade-out.webm` (one-shot) | §15.6 Beat H.5 | Reusable — any beat where Elara's hologram ends a scene |

### 18.5 Beat J (Archives + Two Witnesses Meet) specific

| VFX ID | Effect | Tech | Output | Source beat | Reuse |
|---|---|---|---|---|---|
| `vfx_log5_beam_transfer` | Cyan beam tracing from the brighter Log 5 memory-crystal across the room to the central pedestal's projection well over 3s. Path curves around the top of the domed ceiling's star-chart | PixiJS particle beam + bezier curve path + alpha fade-in/fade-out | `apps/client/public/art/vfx/prelude/log5-beam-transfer.webm` (one-shot) | §17.6 Beat J | Archives only. One-shot per Beat J playthrough |
| `vfx_holo_pedestal_bloom` | Pedestal projection well activates. Cyan rim-glow escalates to bright. Small volumetric bloom rises to ~30cm above the well as the hologram volume initializes. Container for the sepia Prince hologram | Three.js volumetric + CSS keyframe alpha tween | `apps/client/public/art/vfx/prelude/holo-pedestal-bloom.webm` (activation + steady-state variants) | §17.6 Beat J | Archives only. Steady-state runs continuously for the duration of the hologram's presence |
| `vfx_memory_crystal_pulse` | Sub-0.5 Hz cyan pulse on eleven dormant memory-crystals + brighter bright-steady + flaring variant on the Log 5 crystal during Acts 3–8 | CSS keyframe alpha tween on 12 instances with state enum (`dormant` / `bright_steady` / `flaring`) | `apps/client/public/art/vfx/prelude/memory-crystal-pulse.webm` (loop + state variants) | §17.6 Beat J | 12 instances in a circle. Component-ized as `MemoryCrystal` — future Act content may introduce additional crystals using the same component |
| `vfx_enigma_hand_on_rim` | Soft cyan edge-glow where the Enigma's left hand rests on the pedestal's well rim during Movement 4 of Log 5 (~5:20 of the cutscene) | PixiJS localized edge bloom on hand-to-rim contact point | `apps/client/public/art/vfx/prelude/enigma-hand-on-rim.webm` (subtle one-shot) | §17.6 Beat J | Archives only. Fires once. **Subtle** — the player should register that she touched the rim, not that something magical happened |
| `vfx_peripheral_warm_halo` | Warm-amber `#fde68a` halo building around peripheral edges of the frame during Act 10 as *Last Words* enters, holding through the Light/Dark choice, fading on cutscene end | CSS radial gradient mask on outer 15% of frame, animated over 30s | `apps/client/public/art/vfx/prelude/peripheral-warm-halo.webm` (build + hold + fade variants) | §17.6 Beat J | Archives only. Beat J only. VFX signature for *the song entering the room*. Does NOT touch frame-center where the choice UI will appear |
| `vfx_choice_pillar_light_dark_split` | Vertical 2-meter pale pillar appearing at standing eye level center-screen, split vertically: left half soft-gold `#fde68a` (Light), right half deep-violet `#1e1b4b` (Dark). No text, no icons | PixiJS shader with half-mask gradient + alpha fade-in on appearance + half-alpha divergence on player selection | `apps/client/public/art/vfx/prelude/choice-pillar-light-dark-split.webm` (appearance + Light-selection + Dark-selection variants) | §17.6 Beat J | **CANONICAL UI PATTERN FOR ALIGNMENT CHOICES across the entire game.** Register in shared `AlignmentChoiceUI` component in `apps/client/src/components/alignment/`. Full keyboard + gamepad + touch accessibility required |

### 18.6 Ambient audio beds (companion to VFX)

VFX overlays work in concert with ambient audio beds. This subsection catalogs the audio beds for completeness — they are NOT visual assets but they are referenced throughout the VFX sections above and belong in this library for engineering single-source-of-truth.

| Audio bed ID | Description | Output | Source beat | Reuse |
|---|---|---|---|---|
| `ambient_neural_rig_hum` | Soft ~40 Hz low hum suggesting an idle neural interface rig on standby power. Seamless loop | `apps/client/public/audio/ambient/prelude/neural-rig-hum.mp3` (-18 LUFS) | §13.6 Beat G | **§17.4 Beat J** (background layer at 50%). Any Medical Bay re-entry |
