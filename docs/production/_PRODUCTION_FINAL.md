# Loredex OS — _PRODUCTION_FINAL.md

**Single-file production deliverable for AAA-grade asset generation.**

This document is the consolidated, finalized production-doc bundle for the
Loredex OS apprentice + commons + pedagogy + berth + Game Master Act-7
meta-arc rollout. It combines two source documents authored across May 2026
into a single deliverable, reorganized so that **character profiles and room
art** are the entry-point — the gating production batches that every
downstream asset references back to.

```
file:               docs/production/_PRODUCTION_FINAL.md
size:               ~16,000 lines
sections:           7 parts (front matter + 6 part-bodies)
sources:            _PRODUCTION_APPRENTICE_COMMONS.md   (9,745 lines)
                    _PRODUCTION_CUTSCENE_PROMPTS.md     (5,126 lines)
                    + this front matter                  (~1,000 lines)
on-main commits:    5e6c88dd (latest); chain back through 06f218f0,
                    8460500c, 65f42a4d, 762e8e18, f49b0d6e, 37dd15c0,
                    e19c5f29, cfd85335, d9bc3d58, 1f827d56
production status:  FINALIZED — every asset spec is AAA-render-ready
```

---

## §0 AAA generation contract

Every asset prompt below honours this contract verbatim. Production-side, no
prompt is submitted to Nano Banana 2 or Veo 3.1 without all of the following
fields populated.

### §0.1 AAA generation contract — required fields per asset

```yaml
asset_id:           <canonical CDN-safe identifier; per §0.7 naming>
asset_type:         master_face | profile_hero | dialog_headshot | dialog_3qbody
                    | thumbnail | silhouette | expression | activity_sprite
                    | cutscene_start | cutscene_end | cutscene_clip
                    | room_master_still | ui_chrome | overlay | artifact_face
                    | particle_vfx | sfx_bank | music_cue
model:              gemini-3-pro-image-preview            # NB2 stills (4K)
                    | gemini-3.1-flash-image-preview      # NB2 fallback (lower-fidelity)
                    | veo-3.1-generate-001                # Veo 8s clips (1080p)
                    | veo-3.1-fast-generate-001           # Veo fast-path
                    | veo-2.0-generate-exp                # Veo style-transfer fallback
aspect_ratio:       1:1 | 2:3 | 3:4 | 4:3 | 16:9 | 21:9 | 9:16
resolution:         1080x1080 | 1080p | 1536x2048 | 2160p | 4K | 256x256
duration_seconds:   <only for veo>; 4 | 6 | 8 (default 8)
reference_images:   [<≤5 character refs + ≤14 object refs per bundle>]
                    # NB2 character-consistency cap per prompting.systems guide
prompt:             <5-block schema for NB2; 5-part + audio + timestamps for Veo>
negative_prompt:    <Veo only; passed to negativePrompt API param>
nb2_seed:           <deterministic integer; per §0.5 seed namespace>
veo_seed:           <deterministic integer; per §0.5 seed namespace>
vo_manifest_ref:    apps/shared/<character>VoManifest.json#L<line> | null
cdn_target:         cdn/client-public/<canonical-path-per-§0.7>
notes:              <production-side caveats — stitching, fallbacks, runtime
                    composition, etc.>
```

### §0.2 Nano Banana 2 (image) — research-locked schema

Per agent `a239a69c6ef1803b3` (Google Cloud Nano Banana prompting guide +
Vertex AI docs):

```
NB2_MODEL:                 gemini-3-pro-image-preview   # GA Feb 26 2026
NB2_FALLBACK:              gemini-3.1-flash-image-preview
NB2_INPUT_TOKEN_CAP:       131072
NB2_ASPECT_CINEMATIC:      "21:9"          # all cinematic stills
NB2_ASPECT_PORTRAIT:       "2:3"           # profile heroes
NB2_ASPECT_SQUARE:         "1:1"           # dialog headshots, thumbnails
NB2_ASPECT_3QBODY:         "3:4"           # 3/4-body dialog
NB2_RESOLUTION_DEFAULT:    "4K"            # 4K master renders
NB2_RESOLUTION_DIALOG:     "1080x1080"     # dialog UI overlay
NB2_RESOLUTION_THUMB:      "256x256"       # roster grid
NB2_REF_IMAGE_CAP:         6               # ≤5 distinct chars / ≤14 objects per bundle
NB2_NO_NATIVE_NEGATIVE:    true            # negatives expressed semantically only
                                            # in Constraints block (no API param)
```

**5-block schema (mandatory per NB2 prompt):**
1. **SUBJECT** — what is depicted; trait-locked descriptors verbatim from canon
2. **COMPOSITION** — shot type / camera height / focal length / DOF / framing
3. **LIGHTING/CAMERA** — color-temperature in K / lens type / film stock /
   anamorphic flare / volumetric haze
4. **STYLE** — palette hex codes / aesthetic_anchor reference / mood
5. **CONSTRAINTS** — universal NB2 base constraints + FPV trait-lock when
   applicable + per-prompt exclusions

### §0.3 Veo 3.1 (video) — research-locked schema

```
VEO_MODEL:                 veo-3.1-generate-001         # GA
VEO_FAST:                  veo-3.1-fast-generate-001
VEO_DURATION_DEFAULT:      8                            # 4 | 6 | 8 supported
VEO_DURATION_FORCE_8S:     true_when_subject_refs_passed
VEO_ASPECT:                "16:9"
VEO_RESOLUTION:            "1080p"
VEO_REF_IMAGE_CAP:         4                            # subject refs only
VEO_NO_STYLE_REFS:         true                         # 3.1 limitation; fall back
                                                         # to veo-2.0-generate-exp
VEO_STITCH_PATTERN:        "two 8s clips, last_frame_a == first_frame_b"
VEO_NEGATIVE_PROMPT:       supported via negativePrompt API param
```

**5-part schema + Audio + Timestamps (mandatory per Veo prompt):**
1. **CINEMATOGRAPHY** — shot type / camera move + slow/smooth/whip / focal length
2. **SUBJECT** — trait-locked subject descriptors
3. **ACTION** — single primary verb-phrase + secondary beat
4. **CONTEXT** — location / time / weather
5. **STYLE & AMBIANCE** — film stock / palette / mood
6. **AUDIO block** — Dialogue / SFX / Ambient noise / Score (one foreground
   per category; ≤8 words dialogue per 8 s; mark `(lip-sync to dialogue)`
   when manifest VO is referenced)
7. **Timestamp choreography** — `[00:00–00:03] beat A` / `[00:03–00:06] beat B`
   / `[00:06–00:08] beat C, lands on last_frame`

### §0.4 Canonical FPV trait-lock (string; reuse VERBATIM)

`FPV_LOCK_PHRASE_NB2`:
> first-person POV from the player's eyes; only the player's gloved hands
> enter lower frame from below; no third-person body; no mirrors or
> reflections of the player; do not show the player's face or body;
> consistent eye-height per host_space (small 1.20 m, medium 1.65 m,
> tall 1.95 m, xenomorph at avatar-rig eye-bone)

`FPV_LOCK_PHRASE_VEO`:
> POV shot from the protagonist's eyes; first-person; the camera is the
> character's head; only hands and forearms enter frame from below; the
> camera never cuts to third-person; no mirrors or reflective surfaces
> show the player

`VEO_NEGATIVE_PROMPT` (passed verbatim to `negativePrompt` API param):
...
**FPV exceptions** (documented in-line per cutscene):
- HB12 self-duel — across-table opponent IS the player's mirror image
- Singularity (`cs_collectors_arena_first_singularity`) — frozen-radius constraint
- Vortex R3 First Thing That Noticed You — no humanoid in dark patch
- Pool of Tears — no player reflection in pool surface
- Mirror Cipher — subject IS a mirror reflection
- §AC.4.2 Audit Chamber — FPV-of-apprentice (not player) for audit cuts
- §AC.22.2.4 Game Master face reveal — sole canonical player-face third-person
- §AC.22.2.5 Mol'Garath beat — FPV trait-lock relaxed; static lockoff at
  audience-head dais level

### §0.5 Deterministic seed namespace

```
NB2 SEED RANGES:
  140000–149999   Phase G new cutscenes (combat / spatial / narrative)
  150000–159999   Phase G casino + matrix schools + retro narrative
  160000–169999   Phase G retro misc (demon / cloning / Terminus / Cat B / Cat C)
  170000–179999   Apprentice §AC.1 + §AC.2 (initial cohort rooms + cutscenes)
  180000–189999   Apprentice §AC.4 + §AC.5 + §AC.7 (PR #580 expansion)
  190000–199999   Apprentice §AC.10 + §AC.11 + §AC.22 (character art + canon
                  + Act-7 cinematic)

VEO SEED RANGES:
  240000–249999   Phase G Veo (offset by 100000 from NB2)
  250000–259999   Phase G casino + matrix schools + retro
  260000–269999   Phase G retro misc Veo
  270000–279999   Apprentice §AC.2 Veo
  280000–289999   Apprentice §AC.7 Veo
  290000–299999   Apprentice §AC.22 Veo

REFERENCE-IMAGE SEED:
  Master faces use deterministic seeds in 190001..190732 range; downstream
  asset seeds derive from master_face_seed + (asset_type_offset * 1000).
```

### §0.6 Reference-image bundle convention (Nano Banana 2 best practice)

Per Nano Banana 2 character-consistency research:

1. **Master face render first** per character (94 master faces total: 72 apprentice
   variants + 5 recruits + 16 named NPCs + 1 aesthetic anchor).
2. **≤5 distinct characters per ref bundle** in any single NB2 call. For
   ensemble shots (cohort group portrait, Warden Dock 2-character scene)
   this is the cap.
3. **Trait-lock by reusing identical descriptive tokens verbatim** in every
   prompt — this is what produces "same person across every render."
4. **Reference-image bundle for downstream renders**: master face + 1
   environment ref + (optional) 1 wardrobe ref. 3-image bundle is the
   canonical workflow.
...
├── apprentice_aesthetic_anchor.png                                  # gating; render FIRST
├── apprentice_<archetype>_<gender>_<variant>_master_face.png        # 72 variants
├── recruit_<recruit>_master_face.png                                # 5 named recruits
├── warden_veil7_master_face.png
├── archon_<house_id>_master_face.png                                # 12 guild Archons
├── mechronis_auditor_master_face.png                                # brass-mask only
└── quartermaster_master_face.png

cdn/client-public/art/portraits/                                     # downstream 11-asset
├── apprentice_<archetype>_<gender>_<variant>_<asset_type>.png
├── recruit_<recruit>_<asset_type>.png
└── ...

cdn/client-public/art/sprites/                                       # activity sprites
cdn/client-public/art/overlays/                                      # doctrine + memory + trained-crew
cdn/client-public/art/cards/                                         # signature cards
cdn/client-public/art/refs/                                          # location stills
cdn/client-public/art/ui/                                            # UI chrome

cdn/client-public/cutscenes/<cs_id>/
├── start.png                                                        # NB2 21:9 4K
├── end.png                                                          # NB2 21:9 4K
├── clip.mp4                                                         # Veo 8s 1080p
├── audio_post.wav                                                   # ffmpeg mixed
└── meta.json                                                        # seeds + hashes

cdn/client-public/audio/score/                                       # music cues
cdn/client-public/audio/sfx/                                         # SFX bank
cdn/client-public/audio/vo/                                          # VO manifest renders
```

### §0.7 CDN naming convention

```
cs_id pattern:                   cs_<event>_<archetype|location|state>
master_face pattern:             apprentice_<archetype>_<gender>_<variant>_master_face.png
profile_hero pattern:            apprentice_<archetype>_<gender>_<variant>_profile_hero.png
dialog_headshot pattern:         apprentice_<archetype>_<gender>_<variant>_expression_<state>.png
activity_sprite pattern:         apprentice_<archetype>_<phase>_activity.png
doctrine_overlay pattern:        doctrine_<doctrine>_<archetype>_overlay.png
trained_crew_overlay pattern:    trained_crew_<role>_<archetype>_overlay.png
memory_inheritance pattern:      memory_inheritance_<archetype>.png
signature_card pattern:          signature_<archetype>_<effect_slot>_<corruption_band>.png
ui_chrome pattern:               ui_<component>_master.png
room_master_still pattern:       <space_id_short>_master_still.png
score_cue pattern:               music_<context>_<cue_id>.wav
sfx_atomic pattern:              sfx_<descriptor>.wav
```

All paths are case-sensitive lowercase with underscore-separated tokens. Max
255 chars per CDN key (S3 standard).

### §0.8 APPRENTICE_AESTHETIC anchor (steampunk-cyberpunk-occult)

Inline phrase reused **verbatim** across every apprentice-side NB2 + Veo
prompt:

```
APPRENTICE_AESTHETIC: steampunk-cyberpunk-occult; brass armatures and
copper piping with cyber-cyan fiber-optic conduits emerging from brass
fittings, sigil-etched brass plates and chalk-circle floor inlay,
gas-mantle 1800K key with cyber-cyan rim and occult-violet practicals
at sigil hot-spots; palette `#c9a14a / #5fa8ff / #0d0a08 / #ff2a8a /
#5a1a1f / #dccfaa`; Kodak Vision3 500T pushed +1; ARRI Alexa
anamorphic; volumetric oil-smoke + cyber-mist haze z+1.5–3.0m.
```

Forbidden in apprentice-side renders:
- Clean-white sterile surfaces (clinical-sterile is wrong era)
- Pure-digital flat-shaded UIs (cyberpunk surfaces THROUGH brass / leather / candle-wax)
- Pure stone-medieval (medieval here always wears brass armature + fiber-optic conduit)

### §0.9 Camera-spawn parity (FPV eye-height per host_space)

Every NB2 / Veo prompt must honour the `camera_spawn` height declared in
the cross-cut spine:
...
### §0.10 13-state axis grid (every room)

Every space in this document carries the canonical Ark-room 13-state axis
grid (per `INCEPTION_ARK_FINAL_PRODUCTION.md` §2.x.6 back-fill convention):

```
1   architect-layer geometry           fixed (1 state)
2   floor / walls / ceiling / lights   fixed (1 state)
3   atmosphere + sound + smell         fixed (1 state)
4   objects + cameras + doors +        fixed (1 state)
    story-tie + FX + perf budget
5   connection rules                   modulated (per-doorway)
6   economic surface                   3 states (idle / active / contested)
7   governance modifier reactions      modulated (per-modifier flag)
8   tournament / event window          4 states (off / qualifier / finals / champion)
9   TV-infection                       5 states (clean / exposed / spreading /
                                       corrupted / quarantined)
10  epoch / shadowtongue               4 states (low / rising / high / grand-edit)
11  cycle-phase / time-of-day          4 states (dawn / midday / dusk / nightwatch)
12  faction livery                     8 states (none / hierarchy / dreamers /
                                       pureflame / insurgency / panopticon /
                                       collectors / multi)
13  storyteller hooks + HUD overlap    open list (mystery-arc bindings,
                                       investigation tier, expansion-reserved
                                       zones, HUD overlay surfaces)
```

### §0.11 Audio mix contract

```
audio_mix_target:    -14 LUFS (final mix per cutscene)
audio_format:        WAV 96kHz 24-bit (master); MP4-AAC 320kbps for ship
ambient_bed_db:      -32 to -42 dB (per host_space; documented per room)
chant_loop_db:       -28 to -38 dB
dialog_db:           -6 dB peak (vo manifest takes); Veo native dialogue
                     muted in audio post when manifest line exists
sfx_db:              variable; foreground -12 dB peak; one per cut
score_db:            Cat A: -∞ (none); Cat B: -42 dB pad; Cat C: theme-defining
                     full mix; Cat D (Act-7 meta-arc): theme-defining +
                     orchestral commission
reverb_per_space:    documented per room §3 atmosphere + sound (range
                     0.8s pier outdoors → 8.4s Tier-Infinity Chess Hall)
```

### §0.12 Cinematic categories

| cat | source | length | music | FPV | description |
|---|---|---|---|---|---|
| **A** | §3.1 | 6–12 s | none | strict | punctuation / SFX-driven / ≤1 short VO |
| **B** | §3.1.B | 8–15 s | low-atmospheric | strict | ambient room-music allowed; held composition |
| **C** | §3.1.C | 15–25 s discovery / 6–10 s loading | theme-defining | strict | game-mode discovery + loading cinematics |
| **D** | §AC.22 | 160-164 s per branch | theme-defining + orchestral | relaxed per-beat | meta-arc cinematic (Act-7 Game Master only) |

### §0.13 Quality gates

```
NB2_QUALITY_BAR:
  - master face structure consistent across the apprentice's 11-asset bundle
  - face structure cross-bundle consistent (master_face → all variants)
  - text rendering ≤25 chars per visible element (NB2 text-rendering-safe)
  - no extra fingers, no watermark, no studio logo
  - APPRENTICE_AESTHETIC palette adherence (within ±5% on dominant accents)
  - FPV trait-lock string present verbatim when applicable

VEO_QUALITY_BAR:
  - dialog ≤8 words per 8 s clip
  - lip-sync mark `(lip-sync to dialogue)` present when manifest VO referenced
  - audio-post pipeline mutes Veo native dialogue when manifest line exists
  - first_frame and last_frame match the canonical NB2 stills
  - 12s+ stitched: last_frame_a == first_frame_b verified at frame level
  - one foreground SFX + one ambient bed + (optional) one score per clip
  - no motion smear (camera moves tagged slow / smooth / whip)
  - no character drift across stitched clips (subject refs identical)

GATE_BLOCKING_CONDITIONS:
  - master face render missing → all downstream renders BLOCKED for that
    character
  - APPRENTICE_AESTHETIC anchor render missing → all apprentice renders BLOCKED
  - VO manifest line number unresolved → audio post BLOCKED for that line
    (Veo native dialogue cannot ship as canonical for a manifest-referenced line)
```

---

## §1 Document map — character profiles + room art FIRST

This document is reorganized for AAA generation: the gating production
batches (master faces, character profiles, room master stills) appear FIRST.
Cutscenes and downstream assets reference back to these.

### §1.1 Reading order (recommended for production-side asset generation)

```
PART I:    Front matter + AAA generation contract                     (this section)
PART II:   Apprentice + commons + pedagogy + Game Master Act-7         (lines ~1100–10,800)
           Source: _PRODUCTION_APPRENTICE_COMMONS.md (full)
           Internal order:
             §AC.0  Framework + APPRENTICE_AESTHETIC
             §AC.1  Apprentice cohort spaces (12 rooms with 13-axis)
             §AC.2  Apprentice cutscenes (45/59 cuts)
             §AC.3  Initial audit
             §AC.4  Pedagogy + audit + forge spaces (9 rooms with 13-axis)
             §AC.5  Berth system spaces (5 surfaces with 13-axis)
             §AC.6  12 Guild Common Rooms (with 13-axis)
             §AC.7  PR #580 cutscenes (~298 cuts at compact)
             §AC.8  Time-of-day overlay framework
             §AC.9  PR #580 audit
             §AC.10 Character art — 836 portrait/dialog/artifact renders
             §AC.11 Apprentice canon — 12 named characters + 72 face variants
             §AC.12 UI surface art (89 renders)
             §AC.13 Activity sprite NB2 prompts (68 sprites)
             §AC.14 Doctrine-bound outfit overlays (60 variants)
             §AC.15 Consolidated VFX library (~42 effects)
             §AC.16 Consolidated SFX library (~80 effects + 12 cohort chorus)
             §AC.17 Music score spec (25 cues + variants)
             §AC.18 Trained-crew graduation outfit overlays (60 variants)
             §AC.19 Notable Alumni rosters (60 entries)
             §AC.20 Memory-card-recipient visual overlay (12 variants)
             §AC.21 Cross-doc TBDs final reconciliation
             §AC.22 Game Master Act-7 meta-arc cinematic (Cat D; A.73 +
                    10 beats + 3 branches)

PART III:  Phase G cutscene NB2 + Veo prompts (300 cutscenes)         (lines ~10,800–16,000)
           Source: _PRODUCTION_CUTSCENE_PROMPTS.md (full)
           Internal order:
             §G.0   Framework
             §G.1   Pet Arena fighter cutscenes (12)
             §G.2   Chess Hall cutscenes (8)
             §G.3   Boss arena hero-cuts (5)
             §G.4   Castle of Death chamber arrivals (20)
             §G.5   Trade Empire sector first-arrivals (28)
             §G.6   Tower Defense map cutscenes (20)
             §G.7   Vortex Incursion missing rooms (5)
             §G.8   Generic Incursion room cuts (16)
             §G.9   Casino game-tables (6)
             §G.10  Quiz Show Q7–Q12 (6)
             §G.11  Matrix Schools per-episode (24)
             §G.12  Retro narrative cutscenes (68)
             §G.13  Retro misc cutscenes (67)
             §G.F   Phase G audit + production handoff
```

### §1.2 Character profile index — render in this order

**The master-face anchor batch is the gating step.** Generate this first.

| § | character | species | class | seed | CDN |
|---|---|---|---|---|---|
| §AC.0.1 anchor | apprentice_aesthetic_anchor | — | — | n/a | `cdn/client-public/art/refs/apprentice_aesthetic_anchor.png` |

**Apprentices (72 variants — 12 archetypes × 2 genders × 3 variants):**

| § | canon name | species | class | variants | seed range |
|---|---|---|---|---|---|
| §AC.11.1 | Kareth Vael-Drumm | Demagi | Priest | 6 (3F + 3M) | 190001–190002b |
| §AC.11.2 | Sira Null-Echo | Quarchon | Spy/Assassin | 6 (3F + 3M) | 190003–190004b |
| §AC.11.3 | Tien Ceadrune | Human | Scholar | 6 (3F + 3M) | 190005–190006b |
| §AC.11.4 | Bohl-Mor Krellix | Demagi | Soldier | 6 (3M + 3F) | 190007–190008b |
| §AC.11.5 | Pellix Vaun-Brass | Quarchon | Engineer | 6 (3M + 3F) | 190009–190010b |
| §AC.11.6 | Aevel of the Five Gates | Demagi | Oracle | 6 (3F + 3M) | 190011–190012b |
| §AC.11.7 | Roon Calpha | Human | Smuggler | 6 (3M + 3F) | 190013–190014b |
| §AC.11.8 | Iva-Marl Sinder | Demagi | Healer | 6 (3F + 3M) | 190015–190016b |
| §AC.11.9 | Caedex Vorr | Demagi | Warlock | 6 (3M + 3F) | 190017–190018b |
| §AC.11.10 | Vex'rah Halflaugh | Quarchon | Diplomat-Spy | 5 (3NB + 1M + 1F) | 190019–190021 |
| §AC.11.11 | Marcus Farrow | Human | Sentinel | 6 (3M + 3F) | 190022–190023b |
| §AC.11.12 | Lord Avern Thessler | Human | Smuggler-Heir | 6 (3M + 3F) | 190024–190025b |

**Recruits (5 named; 11-asset bundle each):**

| § | recruit | role | seed |
|---|---|---|---|
| §AC.10.7 | Vex Solène | Engineer (Tier-2 recruit) | 190xx |
| §AC.10.7 | Wraith Calder | Survivor (Tier-2 recruit) | 190xx |
| §AC.10.7 | Locke | Adjudicator (Tier-2 recruit) | 190xx |
| §AC.10.7 | Jericho Jones | Trade-broker (Tier-2 recruit) | 190xx |
| §AC.10.7 | Akai Shi | Necromancer (Tier-2 recruit) | 190xx |

**Named NPCs (16 — 12 Archons + Veil-7 + Auditor + Quartermaster + Mol'Garath):**

| § | NPC | seed |
|---|---|---|
| §AC.10.8 | Inspector Veil-7 (the Warden) | 190551–190561 |
| §AC.10.8 | Mechronis Auditor (faceless brass-mask) | 190751–190755 |
| §AC.10.8 | Quartermaster | 190761–190771 |
| §AC.10.8 | Professor Steele (Iron) | 190601–190611 |
| §AC.10.8 | Professor Lenz (Glass) | 190612–190622 |
| §AC.10.8 | Professor Veil (Smoke) | 190623–190633 |
| §AC.10.8 | Professor Quill (Ledger) | 190634–190644 |
| §AC.10.8 | Professor Wirework (Circuit) | 190645–190655 |
| §AC.10.8 | Professor Smoke (Thurible) | 190656–190666 |
| §AC.10.8 | Professor Hammer (Anvil) | 190667–190677 |
| §AC.10.8 | Professor Glass-Mask (Mirror) | 190678–190688 |
| §AC.10.8 | Professor Vine (Garden) | 190689–190699 |
| §AC.10.8 | Professor Bell (Chapel) | 190700–190710 |
| §AC.10.8 | Professor Watch (Tower) | 190711–190721 |
| §AC.10.8 | Professor Sigh (Remnant) | 190722–190732 |
| §AC.22.7 | Mol'Garath (8m brass-and-violet armoured demon-form) | TBD — production-side authoring required |

### §1.3 Room art index — render in this order

After master faces, render room master stills. Each space carries full §4
spec + 13-state axis grid + master-still NB2 prompt.

**Ark rooms (apprentice + commons cohort) — §AC.1:**

| § | space_id | name | size | seed |
|---|---|---|---|---|
| §AC.1.1 | ark.apprentice_hall | The Apprentice Hall | 20×16×5.2m | 170001 |
| §AC.1.2 | ark.trial_hall | The Trial Hall | 14×14×7.2m | 170018 |
| §AC.1.3 | ark.recruit_vestibule | The Recruit Vestibule | 8×6×3.2m | 170035 |
| §AC.1.4 | ark.apprentice_cellblock | The Apprentice Cellblock | 24×6×2.8m + 12 cells | 170052 |
| §AC.1.5 | ark.hellbox_clone_bench | The Hellbox Clone Bench | 6×4×3.6m | 170069 |
| §AC.1.6 | ark.mourning_wall | The Mourning Wall | 12×4×3.2m | 170086 |
| §AC.1.7 | ark.essence_harvest_sanctum | The Essence Harvest Sanctum | 8×8×4.0m | 170103 |
| §AC.1.8 | ark.blood_weave_atrium | The Blood Weave Atrium | 12×12×8.0m | 170120 |
| §AC.1.9 | ark.personal_quest_ledger | The Personal Quest Ledger Room | 10×8×3.2m | 170137 |
| §AC.1.10.1 | ark.social_hub.bar | Social Hub — Bar sub-zone | 4×3×4.5m | 170154 |
| §AC.1.10.2 | ark.social_hub.long_table | Social Hub — Long Table sub-zone | 6×2.4×4.5m | 170158 |
| §AC.1.10.3 | ark.social_hub.alcove | Social Hub — Alcove sub-zone | 3×3×4.5m | 170162 |

**Pedagogy + audit + forge spaces — §AC.4:**

| § | space_id | name | size | seed |
|---|---|---|---|---|
| §AC.4.1 | ark.doctrine_binding_chamber | The Doctrine Binding Chamber | 10×10×5.0m | 180001 |
| §AC.4.2 | ark.audit_chamber | The Audit Chamber (+ Day-21 Warden variant) | 8×6×3.2m + booth | 180101 |
| §AC.4.3 | ark.the_forge | The Forge | 10×8×4.2m | 180201 |
| §AC.4.4 | ark.memory_card_library | The Memory Card Library | 12×8×3.6m | 180501 |
| §AC.4.5 | dest.celebration_school.training_barracks | Celebration Park — Training Barracks | 20×12×6.0m | 180701 |
| §AC.4.6 | dest.celebration_school.triangle_alcove | The Triangle Event Alcove | 6×4×3.6m | 180711 |
| §AC.4.7 | dest.warden_dock | The Warden's Dock | 12×6×open-sky | 180801 |
| §AC.4.8 | ark.mission_briefing_war_room | Mission Briefing War Room | 12×10×4.4m | 180901 |
| §AC.4.9 | ark.post_mission_return_hub | Post-Mission Return Hub (7 sub-zones) | varies | 180918+ |

**Berth system — §AC.5:**

| § | space_id | name | size | seed |
|---|---|---|---|---|
| §AC.5.1 | ark.apprentice_berth.<archetype> | Apprentice Berth × 12 archetypes | 3.66×4.88×2.6m | 181001–181060 |
| §AC.5.2 | ark.recruit_berth.<recruit> | Recruit Berth × 5 named | 3.66×4.88×2.6m | 181125–181149 |
| §AC.5.3 | ark.elara_bridge_berth | Elara's Bridge Berth | 6×5×3.2m | 181150 |
| §AC.5.4 | ark.human_observation_deck | The Human's Observation Deck (4 reveal stages) | 8×6×4.0m | 181160 |
| §AC.5.5 | ark.berth_comm_screen | Bunkroom Comm Screen — diegetic UI fixture | 0.6×0.4m | 181201–181209 |

**Guild Common Rooms — §AC.6 (12 houses):**

| § | space_id | guild | seed |
|---|---|---|---|
| §AC.6.1 | ark.guild_common_room.house_of_iron | Iron | 182001 |
| §AC.6.2 | ark.guild_common_room.house_of_glass | Glass | 182002 |
| §AC.6.3 | ark.guild_common_room.house_of_smoke | Smoke | 182003 |
| §AC.6.4 | ark.guild_common_room.house_of_ledger | Ledger | 182004 |
| §AC.6.5 | ark.guild_common_room.house_of_circuit | Circuit | 182005 |
| §AC.6.6 | ark.guild_common_room.house_of_thurible | Thurible | 182006 |
| §AC.6.7 | ark.guild_common_room.house_of_anvil | Anvil | 182007 |
| §AC.6.8 | ark.guild_common_room.house_of_mirror | Mirror | 182008 |
| §AC.6.9 | ark.guild_common_room.house_of_garden | Garden | 182009 |
| §AC.6.10 | ark.guild_common_room.house_of_chapel | Chapel | 182010 |
| §AC.6.11 | ark.guild_common_room.house_of_tower | Tower | 182011 |
| §AC.6.12 | ark.guild_common_room.house_of_remnant | Remnant | 182012 |

**Game Master Act-7 — §AC.22.1:**

| § | space_id | name | size | seed |
|---|---|---|---|---|
| §AC.22.1 | ark.tier_infinity_chess_hall | The Tier-Infinity Chess Hall (pocket-dimension) | 24×24×16m non-Euclidean | 197001 |

### §1.4 Cutscene index — render after rooms

300 Phase G cutscenes (Part III, §G.1–§G.13) + 357 apprentice cutscenes
(Part II, §AC.2 + §AC.7 + §AC.22.2) = **657 total cutscenes** with full
NB2 + Veo prompts. Cumulative production assets per cutscene per §G.0.4
contract: 5 baseline files (start.png + end.png + clip.mp4 + audio_post.wav
+ meta.json) + variants for stitched 12s + outcome variants.

### §1.5 Cumulative production roster

```
Master faces:               94    (1 anchor + 72 apprentice + 5 recruits + 16 NPCs)
Downstream 11-asset renders: 1,034 (94 × 11)
Artifact-faced renders:      358   (5 doctrine slips + 36 audit transcripts +
                                    29 mission dossiers + 24 memory cards +
                                    216 signature cards + 48 plaques)
UI surface renders:          89    (dialog frame + pedagogy hub + mission
                                    resolver + cohort + warden + comm-screen
                                    9 states + memory library + doctrine
                                    picker + forge + audit transcript +
                                    commons panel)
Activity sprites:            68    (12 archetypes × 4 phases + 5 recruits × 4)
Outfit overlays:             132   (60 doctrine + 60 trained-crew + 12 memory)
Atomic VFX:                  ~42   (~30 base + 12 per-guild)
Atomic SFX:                  ~80   (+ 12 archetype cohort-chorus chords)
Music score cues:            ~25   (+ 12 guild + 7 mission-return + meta-arc 6)
Cutscenes (NB2 + Veo):       657   (300 Phase G + 357 apprentice/cinematic)
Cutscene asset files:        ~3,285 (657 × ~5 baseline)
Spaces (rooms + sub-zones):  38     (12 cohort + 9 pedagogy + 5 berth + 12 guild
                                    + 1 Tier-Infinity)

TOTAL INDIVIDUAL PRODUCTION ASSETS:  ~5,300
TOTAL CDN STORAGE ESTIMATE:          ~35 GB
TOTAL NB2 GENERATION COST (est.):    ~$170
TOTAL VEO GENERATION COST (est.):    ~$2,100
TOTAL ESTIMATED API SPEND:           ~$2,270 one-time
```

### §1.6 Outstanding TBDs (production-blocking)

```
[A] §F.1 cross-cut sync — 460 new cutscene IDs need spine entries in
    _PRODUCTION_CROSS_CUT.md §F.1.A.4-A.16 (separate doc-pass)

[B] VO manifest line numbers — 24 archetype × gender + 17 new NPC manifests
    + 4 pedagogy line-banks (2,496 lines) need lineId mapping (audio post
    blocked until resolved)

[C] 17 new VO manifests (essence harvester, restored apprentice, 12 archons,
    Inspector Veil-7, Mechronis auditor, quartermaster) need authoring
    (dialog system blocked)

[D] Mol'Garath full visual identity (8m tall brass-and-violet armoured
    demon-form) — needs master visual identity authoring before §AC.22.2.5
    + §AC.22.2.10 can render

[E] Player's voice manifest — 12 default voices in apps/shared/playerVoManifests/
    for §AC.22.2.4 fallback (Beat 4 player-face reveal)

[F] Game Master younger-form master_face — age-regression of player's
    canonical face at ~16-17yo for §AC.22.2.6 photograph reveal

[G] 12 prior-Game-Master brass plate names — runtime-parametric from §AC.11
    canonical-name pool

[H] Score commission — full orchestral score for §AC.22 ~3 minutes unique
    material with branches (production audio team timeline; separate from
    this doc)

[I] Tier-4 chess match (§AC.22.2.10 negotiate branch) — Act-7+ DLC reservation;
    out of scope
```
...
# PART II — Apprentice + commons + pedagogy + Game Master Act-7

_(full content of `_PRODUCTION_APPRENTICE_COMMONS.md` follows; 9,745 lines; sections §AC.0 through §AC.22)_
...
# PART III — Phase G cutscene NB2 + Veo prompts (300 cutscenes)

_(full content of `_PRODUCTION_CUTSCENE_PROMPTS.md` follows; 5,126 lines; sections §G.0 through §G.F)_
...
End of `_PRODUCTION_FINAL.md`. Production-side asset-generation pipeline can begin immediately against this document. Master-face anchor batch + 94 master_faces are the gating step (per §0.6 + §1.2).
