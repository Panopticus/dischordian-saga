...
### Build-everything follow-on (stricter policy: "reserved for future = build it now")

After the user invoked the stricter policy, every back-end-only / scaffold-only surface remaining in the audit's wake was built end-to-end:

- **Phase F — Guild Contracts UI** (commit `d26cad0`): new "CONTRACTS" tab on GuildPage backed by the existing 4 procedures of `guildContracts`. Progress accrues server-internally via `incrementContractProgress` from `guild.donate` and `guildWars.contribute`; UI renders bars + CLAIM button + cinematic queue (mirrors GuildTreasury pattern). 10 source-scan tests pin the wiring.
- **Phase G — Duelyst multiplayer client** (commit `9085e42`): new `useDuelystPvpSocket` hook (mirrors PvpArenaPage's `connectWs`) + `DuelystMatchmakingPage` (5-phase UI: lobby/queue/match-found/playing/ended) + DuelystPage menu entry "RANKED MULTIPLAYER" + `/duelyst-pvp` route. The previously-orphan `/api/duelyst-pvp` WebSocket now has a client connector. 31 new tests (19 hook reducer + 12 page composition).
- **Phase H — Engine reserved features** (commit `ec89fe0`): all 10 `// reserved` engine surface members built. 4 already-runtime ops/keywords (`dispel`/`push`/`if`/`structure`) got demo cards; 3 keywords (`zeal`/`pack`/`resurrect`) got engine handlers in `combat.ts` (effectivePower with adjacency + same-defId stacking) and `stateBasedActions.ts` Pass 1a (resurrect intercept); 3 triggers (`on_card_played`/`on_move`/`on_summoned_near_me`) got matcher loops in `playCard.ts` / `movement.ts` / `deploy.ts`. 10 new cards in `engine_demo_cards.ts`, 22 new runtime tests (engineDemoCards 8, zeal 4, pack 4, resurrect 3, triggerMatchers 3). Reserved annotations stripped from `Effect.ts` / `Trigger.ts` / `Card.ts`.
- **Phase I — marketAchievements wiring** (commit `fd23541`): `marketStatsService` aggregates running totals from source tables; `achievementTracker` now dispatches stat-key matches to `unlockMarketAchievementsByStat` which writes to `userAchievements`; new `MarketplaceAchievementsPage` surfaces all 75+ achievements across 7 domains with tier-styled cards. The `@deprecated` header on `marketAchievements` was replaced by a functional one.
- **Phase J1 — Suit-materials inventory** (commit `c6c6a57`): `citizenCharacters.suitMaterials: json` column added; `attemptSuitCraft` now gates against the pouch with a `missing_materials` outcome and deducts on success.
- **Phase J2 — Trade contract effect interpreter** (commit `c6c6a57`): all 10 `ContractClauseEffect` kinds now handled at signing — `trust_delta` upserts npcTrust; `set_flag` merges into `userProgress.gameData.narrativeFlags`; `faction_reputation_delta` merges into `factionReputation`; `reward_modifier` composes via new `tradeContracts.rewardModifierPct` column. Phase-3 broker effects (cross_reference / aleatory_roll / market_volatility / ceremonial_audit) deferred to stage progression with explicit logging. Exhaustiveness `never` check at the bottom.
- **Phase J3+J4 — Engine "not yet implemented" stubs** (commit `d040bcd`): all 5 unimplemented condition kinds (`self_damaged`/`self_at_health`/`allies_with`/`mana_available`/`target_exists`), all 3 unimplemented amount kinds (`x_cost`/`missing_health`/`counter_of`), and all 4 unimplemented selector kinds (`radius`/`line`/`nearest`/`position_empty`) implemented with proper helpers (`resolveOrigin`, `directionToDelta`, `countBoardMatches`).
- **Phase J5/J6/J7 surveyed**: J5 (cross-game narrative threads) was already shipping with 9 registered threads; J6 (narrative flag orphans) is a deliberate KNOWN_ORPHANS design surface guarded by `narrativeFlagAudit.test.ts:220`; J7 (E2E auth env var) is an ops/CI configuration task documented in `apps/e2e/README.md`.
- **Phase K — Clue lore for Acts 1-2** (this commit): new `apps/shared/conspiracyBoards/clueLore.ts` with 2-3 sentence body text + LOREDEX entity anchors for all 10 clues authored in C3. Renderer-agnostic (label-only fallback for Acts 3-7 stays). 5 registry-shape tests.
- **Phase L — Env-driven Stripe price IDs** (this commit): `StoreProduct.stripePriceEnv` field reads from `process.env[name]` at checkout time; falls back to inline `price_data` from `priceUsd` when the env var is unset. `.env.example` documents `STRIPE_PRICE_FOUNDING_AUTHOR` and `STRIPE_PRICE_AUTHORS_EDITION_S2`.
- **Phase M — Python pipeline `--dry-run`** (this commit): `generate_act1_taunts_vo.py` now supports `--dry-run` for structural validation without `boto3` / `requests` / API keys installed. Reports per-line readiness (would-generate / unknown-character / TODO-voice / unknown-emotion). All 21 lines validate ✓.
- **Phase N (this commit)**: this section, plus the per-phase commit hash trail.

### Final tally

What the original audit flagged as "hidden" or "reserved" is now either built and shipping (F–M), already shipping under a different name (J5, the alias-mapped routers), or by-design deferred (J6, J7). The CI guard from C5 prevents future regressions. 12 commits, ~3,000 new lines, 70+ new tests (all green), zero typecheck errors, zero lint regressions.
