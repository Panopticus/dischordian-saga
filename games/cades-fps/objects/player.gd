...
func damage(amount):
	...
	if health < 0:
		if GameMode.current_mode == "last_stand":
			# Send result before loop reset
			WebBridge.send_result({
				"mode": "last_stand",
				"time_held": GameMode.time_held,
				"waves_survived": GameMode.current_wave,
				"loop_count": GameMode.loop_count,
				"awareness_level": GameMode.awareness_level,
				"canon_achieved": GameMode.canon_achieved,
			})
			LoopManager.on_player_death()
		else:
			get_tree().reload_current_scene()
